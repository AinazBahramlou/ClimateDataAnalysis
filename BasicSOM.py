...
SumInteria=0
for s in range(1,11):
    som = SOM(m=M*D, n=1, dim=F, random_state=s,max_iter=Y*D*E)
    som.fit(dataS,shuffle=True,epochs=E)
    SumInteria+=som._inertia_
    predictions = som.predict(dataS)
    
    (a,b,c)=homogeneity_completeness_v_measure(labels,predictions)
    V.append(a)
    V.append(b)
    V.append(c)
