...
class SOM():
    ...
    """
    def __init__(self, List, m=3, n=3, dim=3, lr=1, sigma=1, max_iter=3000,
                    random_state=None,c=1,flag=0):
        """
        Parameters
        ----------
        m : int, default=3
    ...
    def _find_bmu(self, x):
        ...
        if self.flag==0:
          distance=[]
          for e in range(0,self.m*self.n):
              distance.append(dtw.distance_fast(x_stack[e,:],self.weights[e,:]))
        else: distance = np.linalg.norm(x_stack - self.weights, axis=1)
        #print(distance)
        #print("******************")
    ...
    def step(self, x):
        ...
        #print(bmu_distance)
        if self.flag==0:
           bmu_distance=np.zeros(self.m*self.n)
           for e in range(0,self.m*self.n):
              bmu_distance[e]=dtw.distance_fast(self._locations[e].astype(np.float64),stacked_bmu[e].astype(np.float64))
        else: bmu_distance = np.sum(np.power(self._locations.astype(np.float64) - stacked_bmu.astype(np.float64), 2), axis=1)
        ...
        if self.consensus==1:
           delta = (local_multiplier)*(x_stack-self.weights)+self.weights
           self.weights=delta
        else:
           self.weights=(local_multiplier)*(x_stack-self.Z)+self.Z
           self.consensus=1
...
data= np.genfromtxt('trainX.csv', delimiter=',')
dataY= np.genfromtxt('testX.csv', delimiter=',')
labels= np.genfromtxt('ListX.csv', delimiter=',')
labelsY= np.genfromtxt('ListY.csv', delimiter=',')

scaler1=preprocessing.StandardScaler().fit(data)
dataS=scaler1.transform(data)
data=dataS
data2=pca.fit(dataS,labels).transform(dataS)
scaler2=preprocessing.StandardScaler().fit(dataY)
dataSY=scaler2.transform(dataY)
dataY=dataSY
dataSY=pca.fit(dataSY,labelsY).transform(dataSY)

M=5#5,10,15,20
N=1 
D=1 #Number of partitions: 1-4-9
F=24 #Number of features
Y=365 #partition size
E=1 #epochs
SumInteria=0
SumAcc=0
def task(newList1,d):
    som= SOM(newList1, m=M, n=N, dim=F, random_state=s,max_iter=Y*E)
    data1=data[d*Y:d*Y+Y,:]
    som.fit(data1,epochs=E)
    Z=som.cluster_centers_
    U=np.zeros((M*N,F))
    counter=0
    for i in range(0,M):
      for j in range(0,N):
        U[counter,:]=np.array(Z[i,j,:])
        counter+=1
    return U
V=[]
Z=[]
for s in range(1,11):
 U_List=[]
 newList1=np.zeros((M*N,F))
 with Pool() as pool:
   items=[(newList1,d) for d in range(0,D)]
   for U in pool.starmap(task,items):
     U_List.append(U)
  
 List=[]
 #Combine the clusters
 for l in range(0,len(U_List)):
    for h in range(0,M*N):
        U=U_List[l]
        List.append(U[h,:])  
 
 ListS=scaler1.transform(List)
 List2=pca.fit(dataS,labels).transform(ListS)
 som = SOM(List2, m=M*D, n=N, dim=2, random_state=s,max_iter=len(data2)*1,c=0,flag=1)
 som.fit(data2,epochs=1,shuffle=True)
 SumInteria+=som._inertia_
 # Assign each datapoint to its predicted cluster
 pred = som.predict(data2)
 P = som.predict(dataSY)
 
 count=np.zeros((1000,50))
 for i in range(0,len(pred)):
   count[int(pred[i]),int(labels[i])]+=1
 maxxN=np.zeros(1000)  
 ...
 for l in range(0,len(P)):
   P[l]=maxxN[int(P[l])]
 SumAcc+=met.accuracy_score(labelsY,P)

 
 (a,b,c)=homogeneity_completeness_v_measure(labels,pred)
 V.append(a)
 V.append(b)
 V.append(c)
 (a,b,c)=homogeneity_completeness_v_measure(labelsY,P)
 Z.append(a)
 Z.append(b)
 Z.append(c)
DF=pd.DataFrame(V)
...
DF=pd.DataFrame(Z)
DF.to_csv("d-.csv")
print(round(100*(SumAcc/10)))
#print(SumInteria)
end=time.perf_counter()
print(round(100*((end-start)/10))/100)
